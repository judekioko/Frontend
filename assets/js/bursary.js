...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Prevent implicit form submit via Enter key
    const formEl = document.getElementById('bursaryApplicationForm');
    if (formEl) {
    ...
    // Remove wizard behavior: no injected controls, no step gating

    // Auto-save field values to sessionStorage (keep this improvement)
    const persistIds = Array.from(formEl ? formEl.querySelectorAll('input, select, textarea') : []).map(el => el.id).filter(Boolean);
    function saveAll() {
        const data = {};
        ...
        Utils.saveToStorage('bursaryFormDraft', data);
    }
    function restoreAll() {
        const data = Utils.getFromStorage('bursaryFormDraft') || {};
    ...
    if (formEl) {
        ...
        formEl.addEventListener('change', saveAll);
    }
